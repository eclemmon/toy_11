...
void setup() {
  ...
  // OSC Messages
  OSCMessage error_msg("/player2/error/");
...
void loop() {
  ...
  OSCMessage mpu_msg("/player2/mpu/");
  mpu_msg.add(Ax);
  mpu_msg.add(Ay);
  mpu_msg.add(Az);
  ...
  OSCMessage apds_msg("/player2/apds/");
  apds_msg.add(r);
  apds_msg.add(g);
  apds_msg.add(b);
  apds_msg.add(c);
  ...
  OSCMessage tof1_msg("/player2/tof1/");
  ...
  OSCMessage tof2_msg("/player2/tof2/");
  ...
  // tof1_msg.add(results1.range_status);
  // tof1_msg.add(results1.distance_mm);
  // Udp.beginPacket(IP, LOCALPORT);
  // tof1_msg.send(Udp);
  // Udp.endPacket();
  // tof1_msg.empty();
  ...
  // tof2_msg.add(results2.range_status);
  // tof2_msg.add(results2.distance_mm);
  // Udp.beginPacket(IP, LOCALPORT);
  // tof2_msg.send(Udp);
  // Udp.endPacket();
  // tof2_msg.empty();
